...
module light_show(                  //定义输入输出端口
    input       I_clk,              //系统时钟
    input       I_rst_n,            //复位信号
    input       [15:0] I_show_num,   //输入8-bit数据
    output      [6:0] O_led,        //七段数码管LED信号
    output      [3:0] O_dx          //七段数码管段选信号
    );
    ...
    begin
        ...
        else if(R_dx_temp == 4'b0001 && R_counter >= C_COUNTER_NUM)
        begin       //显示高4-bit数据
            R_temp <= I_show_num[7:4];
            R_dx_temp <= 4'b0010;
            R_counter <= 0;
        end
        else if(R_dx_temp == 4'b0010 && R_counter >= C_COUNTER_NUM)
        begin       //显示低4-bit数据
            R_temp <= I_show_num[11:8];
            R_dx_temp <= 4'b0100;
            R_counter <= 0;
        end
        else if (R_dx_temp == 4'b0100 && R_counter >= C_COUNTER_NUM) 
        begin
            R_temp <= I_show_num[15:12];
            R_dx_temp <= 4'b1000;
            R_counter <= 0;
        end
        else if (R_dx_temp == 4'b1000 && R_counter >= C_COUNTER_NUM)
        begin
            R_temp <= I_show_num[3:0];
            R_dx_temp <= 4'b0001;
            R_counter <= 0;
